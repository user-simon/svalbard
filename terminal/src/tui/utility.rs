...
impl<W: Widget> Widget for CenteredWidget<W> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        let offset = area.height.checked_sub(self.height)
            .map(|d| d / 2)
            .unwrap_or(0);
...
/// `Paragraph` doesn't allow inspecting the number of produced lines (the "height") for dynamic
/// layouts.
...
impl<'a> Widget for WrappedString<'a> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        {
            buf.set_stringn(area.x, area.y + i as u16, line, area.width as usize, self.style);
        }
    }
}
