...
pub fn password(key: &str, pepper: &[u8], seed: &Seed) -> String {
    ...
    let digest = {
        ...
        config.hash_length = 4.max(target_len * 3) as u32;
    ...
    let char_seeds = digest.chunks_exact(3)
        .map(|chunk| {
            if let &[outer_seed, inner_seed, pos_seed] = chunk {
                (outer_seed, inner_seed, pos_seed)
            } else {
                unreachable!()
            }
        });
    // (set_seed, char_seed, pos_seed);
...
mod tests {
    ...
    fn password() {
        let mut seed = Seed {
            ...
            length: 5,
            salt: 0,
            characters: crate::Characters::all(),
            username: None,
        };
        
        for i in 10..20 {
            seed.length = i;
            dbg!(super::password("aaa", b"", &seed));
        }
